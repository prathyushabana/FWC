...
int linalg_norm1(double **a, int m)
{
int i;
double norm=0.0;
...
double **line_intersect(double **a, double **b,double c1,double c2,int m, int n)
{
	double **c;
	c = createMat(m,n);
	double det=((a[0][0])*(b[1][0]))-((a[1][0])*(b[0][0]));
	c[0][0]=(((b[1][0])*c1)-((a[1][0])*c2))/(det);
	c[1][0]=(((a[0][0])*c2)-((b[0][0])*c1))/(det);
	return c;
}
