...
\begin{document}
\begin{center}
\textbf\large{CHAPTER-11 \\ THREE DIMENSIONAL GEOMETRY}
...
\item Find the shortest distance between the lines whose vector equations are\\ $\overrightarrow{r}=(\hat{i}+2\hat{j}+3\hat{k})+\lambda(\hat{i}-3\hat{j}+2\hat{k})$ and\\$\overrightarrow{r}=4\hat{i}+5\hat{j}+6\hat{k}+\mu(2\hat{i}+3\hat{j}+\hat{k})$
